...
// @version      0.2.0
// @description  Рассылка отдельных писем по вашим шаблонам после запуска. Доступен режим черновиков.
...
(() => {
  ...
  function yandexHasPendingRecipient(root, isVisible) {
    return candidates(root, isVisible).some((candidate) => {
      ...
      const text = element.tagName === "INPUT" || element.tagName === "TEXTAREA" ? element.value : pendingText(element);
  ...
  async function fillLetter(account, letter, signal, stillActive = () => true) {
    ...
    for (const address of letter.to) {
      ...
      guard();
      recipient.focus();
      insertRecipient(recipient, address);
      enter(recipient);
      if (account.provider === "yandex") subject.focus();
      await waitFor(
...
${originalSignature}` : "");
    ...
    const verify = () => {
      ...
      if (body.textContent !== text) throw new EditorError("body_mismatch");
    };
    verify();
    return {
      root,
  ...
  function notificationState(prepared) {
    ...
    for (const element of prepared.root.ownerDocument.querySelectorAll(
      ...
      if (prepared.body.contains(element) || element.contains(prepared.body) || element.closest(
        '[contenteditable="true"], [contenteditable="plaintext-only"]'
      ))
        continue;
      if (shown(element)) states.set(element, normalize2(element.textContent));
    }
    return states;
  }
  ...
  async function sendLetter(prepared, signal, beforeClick) {
    let claimed = false;
    let observer;
    try {
      signal.throwIfAborted();
      ...
      const observeSuccess = () => {
        if (!claimed) return;
        for (const [node, text] of notificationState(prepared)) {
          if (sentNotice.test(text) && baseline.get(node) !== text)
            acknowledged = true;
        }
      };
      ...
      while (Date.now() < sentDeadline) {
        signal.throwIfAborted();
        observeSuccess();
        if (acknowledged && editorGone(prepared)) return;
        await new Promise((resolve) => setTimeout(resolve, 150));
      }
  ...
  var BrowserTransport = class {
    constructor(account, deliveryMode = "draft") {
    ...
    async prepare(letter, signal, progress) {
      ...
      let opened = false;
      let sendingSeen = false;
      const sendingStarted = () => {
        if (sendingSeen) return true;
        try {
          sendingSeen = Boolean(store.get(RECEIPT_PREFIX + id));
        } catch {
          sendingSeen = opened && this.deliveryMode === "send";
        }
        return sendingSeen;
      };
      try {
        ...
        if (!tab) throw new TransportFailure("Tampermonkey \u043D\u0435 \u043E\u0442\u043A\u0440\u044B\u043B \u0432\u043A\u043B\u0430\u0434\u043A\u0443.");
        opened = true;
        ...
        while (Date.now() < deadline) {
          ...
          if (!current)
            throw new TransportFailure("\u0417\u0430\u0434\u0430\u043D\u0438\u0435 \u043E\u0447\u0438\u0449\u0435\u043D\u043E \u0438\u043B\u0438 \u043E\u0442\u043C\u0435\u043D\u0435\u043D\u043E.");
          if (current.state === "sending") {
            if (!sendingSeen) progress?.("sending");
            sendingSeen = true;
          }
          if (current.state === "uncertain") throw new UncertainError();
          if (current.state === "done") {
            if (current.result === "sent") {
              sendingSeen = true;
              return "sent";
            }
            if (this.deliveryMode === "send")
              throw new TransportFailure(
                "\u0412\u043A\u043B\u0430\u0434\u043A\u0430 \u043F\u043E\u0434\u0433\u043E\u0442\u043E\u0432\u0438\u043B\u0430 \u0447\u0435\u0440\u043D\u043E\u0432\u0438\u043A, \u043D\u043E \u043D\u0435 \u043F\u043E\u0434\u0442\u0432\u0435\u0440\u0434\u0438\u043B\u0430 \u043E\u0442\u043F\u0440\u0430\u0432\u043A\u0443. \u041E\u0431\u043D\u043E\u0432\u0438\u0442\u0435 \u0441\u043A\u0440\u0438\u043F\u0442 \u0432\u043E \u0432\u0441\u0435\u0445 \u0432\u043A\u043B\u0430\u0434\u043A\u0430\u0445 \u043F\u043E\u0447\u0442\u044B."
              );
            return;
          }
          if (current.state === "error")
            throw new TransportFailure(current.error || "\u041E\u0448\u0438\u0431\u043A\u0430 \u0437\u0430\u043F\u043E\u043B\u043D\u0435\u043D\u0438\u044F.");
          if (tab.closed) throw new TransportFailure("\u0412\u043A\u043B\u0430\u0434\u043A\u0430 \u0437\u0430\u043A\u0440\u044B\u0442\u0430.");
          await pause();
        }
        throw new TransportFailure(
          this.deliveryMode === "send" ? "\u0412\u043A\u043B\u0430\u0434\u043A\u0430 \u043D\u0435 \u043F\u043E\u0434\u0442\u0432\u0435\u0440\u0434\u0438\u043B\u0430 \u043E\u0442\u043F\u0440\u0430\u0432\u043A\u0443 \u0437\u0430 90 \u0441\u0435\u043A\u0443\u043D\u0434." : "\u0412\u043A\u043B\u0430\u0434\u043A\u0430 \u043D\u0435 \u043F\u043E\u0434\u0442\u0432\u0435\u0440\u0434\u0438\u043B\u0430 \u0437\u0430\u043F\u043E\u043B\u043D\u0435\u043D\u0438\u0435 \u0437\u0430 90 \u0441\u0435\u043A\u0443\u043D\u0434."
        );
      } catch (error) {
        if (error instanceof UncertainError || sendingStarted())
          throw new UncertainError();
        const message = signal.aborted ? "\u041E\u0447\u0435\u0440\u0435\u0434\u044C \u043E\u0441\u0442\u0430\u043D\u043E\u0432\u043B\u0435\u043D\u0430. \u041F\u0440\u043E\u0432\u0435\u0440\u044C\u0442\u0435 \u0443\u0436\u0435 \u043E\u0442\u043A\u0440\u044B\u0442\u0443\u044E \u0432\u043A\u043B\u0430\u0434\u043A\u0443." : error instanceof TransportFailure ? error.message : "\u041E\u0448\u0438\u0431\u043A\u0430 \u043F\u043E\u0434\u0433\u043E\u0442\u043E\u0432\u043A\u0438.";
        ...
        try {
          ...
          if (sendingStarted()) throw new UncertainError();
          throw new AttemptedError(
            "\u041D\u0435 \u0443\u0434\u0430\u043B\u043E\u0441\u044C \u043E\u0447\u0438\u0441\u0442\u0438\u0442\u044C \u0432\u0440\u0435\u043C\u0435\u043D\u043D\u043E\u0435 \u0437\u0430\u0434\u0430\u043D\u0438\u0435. \u041F\u0440\u043E\u0432\u0435\u0440\u044C\u0442\u0435 \u0443\u0436\u0435 \u043E\u0442\u043A\u0440\u044B\u0442\u0443\u044E \u0432\u043A\u043B\u0430\u0434\u043A\u0443."
          );
        }
      }
    }
  ...
  async function consumeJob(id) {
    ...
    if (!job || !Number.isFinite(job.expires) || job.expires <= Date.now() || job.state !== "waiting" || store.get(receiptKey))
      return;
    ...
    const liveJob = () => {
      const current = store.get(key);
      return current && current.owner === job.owner && current.id === job.id && current.expires > Date.now() && (current.state === "claimed" || current.state === "sending") ? current : void 0;
    };
    const watcher = setInterval(() => {
      try {
        if (!liveJob()) abort.abort();
      } catch {
        abort.abort();
      }
    }, 200);
    try {
      ...
      if (job.deliveryMode === "send") {
        await sendLetter(prepared, abort.signal, () => {
          abort.signal.throwIfAborted();
          const current = liveJob();
          if (!current || current.state !== "claimed" || store.get(receiptKey))
            throw new Error("Send claim unavailable");
          const receipt = {
            id,
            expires: job.expires,
            state: "sending"
          };
          store.set(receiptKey, receipt);
          committed = true;
          const savedReceipt = store.get(receiptKey);
          if (savedReceipt?.id !== id || savedReceipt.state !== "sending")
            throw new Error("Send receipt unavailable");
          const claimed = liveJob();
          if (!claimed || claimed.state !== "claimed")
            throw new Error("Send claim cancelled");
          store.set(key, { ...claimed, state: "sending" });
          if (liveJob()?.state !== "sending")
            throw new Error("Send claim not saved");
          abort.signal.throwIfAborted();
        });
        if (!liveJob()) throw new UncertainError();
        store.set(receiptKey, {
          id,
          expires: job.expires,
          state: "sent"
        });
        showWorkerNote("\u041F\u043E\u0447\u0442\u043E\u0432\u044B\u0439 \u043A\u043B\u0438\u0435\u043D\u0442 \u043F\u043E\u0434\u0442\u0432\u0435\u0440\u0434\u0438\u043B \u043E\u0442\u043F\u0440\u0430\u0432\u043A\u0443 \u043F\u0438\u0441\u044C\u043C\u0430.", notes);
        if (!liveJob()) throw new UncertainError();
        store.set(key, { ...job, state: "done", result: "sent" });
      } else {
        if (!liveJob()) abort.signal.throwIfAborted();
        showWorkerNote(
          "\u041F\u043E\u043B\u044F \u0437\u0430\u043F\u043E\u043B\u043D\u0435\u043D\u044B. \u041F\u0440\u043E\u0432\u0435\u0440\u044C\u0442\u0435 \u043F\u0438\u0441\u044C\u043C\u043E; \u043E\u0442\u043F\u0440\u0430\u0432\u043A\u0430 \u2014 \u0432\u0440\u0443\u0447\u043D\u0443\u044E.",
          notes
        );
        if (liveJob()) store.set(key, { ...job, state: "done", result: "draft" });
      }
    } catch (cause) {
      const uncertain = committed || cause instanceof SendUncertainError || cause instanceof UncertainError;
      const error = uncertain ? new UncertainError().message : cause instanceof SendError ? new SendError(cause.code).message : workerErrorMessage(cause, abort.signal);
      try {
        if (uncertain)
          store.set(receiptKey, {
            id,
      ...
      try {
        showWorkerNote(error, [
          uncertain ? "\u041E\u0447\u0435\u0440\u0435\u0434\u044C \u043E\u0441\u0442\u0430\u043D\u043E\u0432\u043B\u0435\u043D\u0430. \u0410\u0432\u0442\u043E\u043C\u0430\u0442\u0438\u0447\u0435\u0441\u043A\u043E\u0433\u043E \u043F\u043E\u0432\u0442\u043E\u0440\u0430 \u044D\u0442\u043E\u0433\u043E \u043F\u0438\u0441\u044C\u043C\u0430 \u043D\u0435 \u0431\u0443\u0434\u0435\u0442." : "\u041D\u0438\u0447\u0435\u0433\u043E \u043D\u0435 \u043E\u0442\u043F\u0440\u0430\u0432\u043B\u0435\u043D\u043E. \u041F\u043E\u043B\u044F \u0434\u043B\u044F \u0440\u0443\u0447\u043D\u043E\u0433\u043E \u043A\u043E\u043F\u0438\u0440\u043E\u0432\u0430\u043D\u0438\u044F \u0434\u043E\u0441\u0442\u0443\u043F\u043D\u044B \u0432 \u0438\u0441\u0445\u043E\u0434\u043D\u043E\u0439 \u043F\u0430\u043D\u0435\u043B\u0438."
        ]);
      } catch {
      }
    } finally {
...
${values["\u0424\u0418\u041E"]}`
  ...
  function createLauncher(createPanel, doc = document) {
    ...
    trigger.type = "button";
    const label = "\u2197 \u041E\u0431\u0440\u0430\u0449\u0435\u043D\u0438\u044F \u043F\u043E \u041F\u0414 \xB7 0.2.0";
    ...
    try {
      GM_registerMenuCommand(
        "\u041E\u0431\u0440\u0430\u0449\u0435\u043D\u0438\u044F \u043F\u043E \u041F\u0414 \xB7 0.2.0 \u2014 \u043E\u0442\u043A\u0440\u044B\u0442\u044C",
        () => void open()
      );
    } catch {
    }
  ...
  async function mountPanel(onClose) {
    ...
    heading.append(
      ...
      el(
        "p",
        "\u0412\u044B\u0431\u0435\u0440\u0438\u0442\u0435 \u043A\u043E\u043C\u043F\u0430\u043D\u0438\u0438 \u0438 \u043F\u0440\u043E\u0432\u0435\u0440\u044C\u0442\u0435 \u0442\u0435\u043A\u0441\u0442. \u0421\u043A\u0440\u0438\u043F\u0442 \u043E\u0442\u043F\u0440\u0430\u0432\u0438\u0442 \u043F\u0438\u0441\u044C\u043C\u0430 \u0438\u0437 \u0432\u0430\u0448\u0435\u0439 \u043F\u043E\u0447\u0442\u044B \u043F\u043E\u0441\u043B\u0435 \u0437\u0430\u043F\u0443\u0441\u043A\u0430.",
        "muted"
      )
    );
    header.append(
    ...
    left.append(
      el(
        "p",
        "\u0424\u0418\u041E, email \u0438 \u0437\u0430\u043F\u043E\u043B\u043D\u0435\u043D\u043D\u044B\u0435 \u0434\u043E\u043F\u043E\u043B\u043D\u0438\u0442\u0435\u043B\u044C\u043D\u044B\u0435 \u0434\u0430\u043D\u043D\u044B\u0435 \u0441\u043E\u0445\u0440\u0430\u043D\u044F\u044E\u0442\u0441\u044F \u0432 Tampermonkey \u0434\u043E \u043D\u0430\u0436\u0430\u0442\u0438\u044F \xAB\u041E\u0447\u0438\u0441\u0442\u0438\u0442\u044C \u0434\u0430\u043D\u043D\u044B\u0435\xBB. \u0412 \u043D\u043E\u0432\u043E\u0439 \u0432\u043A\u043B\u0430\u0434\u043A\u0435 \u0434\u0430\u0442\u0430 \u043E\u0431\u0440\u0430\u0449\u0435\u043D\u0438\u044F \u2014 \u0441\u0435\u0433\u043E\u0434\u043D\u044F\u0448\u043D\u044F\u044F. \u0418\u0437\u043C\u0435\u043D\u0435\u043D\u0438\u044F \u0448\u0430\u0431\u043B\u043E\u043D\u0430 \u0434\u0435\u0439\u0441\u0442\u0432\u0443\u044E\u0442 \u0442\u043E\u043B\u044C\u043A\u043E \u0432 \u0442\u0435\u043A\u0443\u0449\u0435\u0439 \u0432\u043A\u043B\u0430\u0434\u043A\u0435. \u041F\u043E\u0434\u043F\u0438\u0441\u0438 \u0438 \u0432\u043B\u043E\u0436\u0435\u043D\u0438\u044F \u0434\u043E\u0431\u0430\u0432\u043B\u044F\u044E\u0442\u0441\u044F \u0432\u0440\u0443\u0447\u043D\u0443\u044E.",
        "muted"
      )
    );
    ...
    function preview() {
      ...
      deliveryNote.textContent = autoSend ? "\u041A\u043D\u043E\u043F\u043A\u0430 \xAB\u041E\u0442\u043F\u0440\u0430\u0432\u0438\u0442\u044C\xBB \u0437\u0430\u043F\u0443\u0441\u043A\u0430\u0435\u0442 \u0440\u0430\u0441\u0441\u044B\u043B\u043A\u0443 \u0432\u0441\u0435\u043C \u0432\u044B\u0431\u0440\u0430\u043D\u043D\u044B\u043C \u043A\u043E\u043C\u043F\u0430\u043D\u0438\u044F\u043C. \u041E\u0442\u043F\u0440\u0430\u0432\u043B\u044F\u0435\u0442\u0441\u044F \u0442\u0435\u043A\u0441\u0442 \u0438\u0437 \u043F\u0440\u0435\u0434\u043F\u0440\u043E\u0441\u043C\u043E\u0442\u0440\u0430, \u0431\u0435\u0437 \u0434\u043E\u0431\u0430\u0432\u043B\u0435\u043D\u0438\u044F \u0444\u0430\u0439\u043B\u043E\u0432 \u0438 \u043F\u043E\u0434\u043F\u0438\u0441\u0438 \u0434\u043E\u043A\u0443\u043C\u0435\u043D\u0442\u0430. \u0415\u0441\u043B\u0438 \u043E\u043D\u0438 \u043D\u0443\u0436\u043D\u044B, \u0432\u044B\u0431\u0435\u0440\u0438\u0442\u0435 \xAB\u0422\u043E\u043B\u044C\u043A\u043E \u0447\u0435\u0440\u043D\u043E\u0432\u0438\u043A\u0438\xBB. \u041F\u0440\u043E\u043F\u0443\u0441\u043A\u0438 \u0434\u043E\u043F\u043E\u043B\u043D\u0438\u0442\u0435\u043B\u044C\u043D\u044B\u0445 \u0440\u0435\u043A\u0432\u0438\u0437\u0438\u0442\u043E\u0432 \u043D\u0435 \u043E\u0441\u0442\u0430\u043D\u0430\u0432\u043B\u0438\u0432\u0430\u044E\u0442 \u043E\u0442\u043F\u0440\u0430\u0432\u043A\u0443." : "\u0421\u043A\u0440\u0438\u043F\u0442 \u0437\u0430\u043F\u043E\u043B\u043D\u0438\u0442 \u043F\u0438\u0441\u044C\u043C\u0430 \u0432 \u043E\u0442\u0434\u0435\u043B\u044C\u043D\u044B\u0445 \u0432\u043A\u043B\u0430\u0434\u043A\u0430\u0445. \u0414\u043E\u0431\u0430\u0432\u044C\u0442\u0435 \u043D\u0443\u0436\u043D\u044B\u0435 \u0432\u043B\u043E\u0436\u0435\u043D\u0438\u044F \u0438 \u043F\u043E\u0434\u043F\u0438\u0441\u044C, \u0437\u0430\u0442\u0435\u043C \u043E\u0442\u043F\u0440\u0430\u0432\u044C\u0442\u0435 \u0438\u0437 \u043F\u043E\u0447\u0442\u044B \u0432\u0440\u0443\u0447\u043D\u0443\u044E.";
    ...
    function update() {
      ...
      progress.textContent = `${queueDelivery === "send" ? "\u041E\u0442\u043F\u0440\u0430\u0432\u043B\u0435\u043D\u043E" : "\u041F\u043E\u0434\u0433\u043E\u0442\u043E\u0432\u043B\u0435\u043D\u043E"} ${done} \u0438\u0437 ${queue.items.length}. ${running ? "\u041D\u0435 \u0437\u0430\u043A\u0440\u044B\u0432\u0430\u0439\u0442\u0435 \u0438\u0441\u0445\u043E\u0434\u043D\u0443\u044E \u0432\u043A\u043B\u0430\u0434\u043A\u0443." : "\u041E\u0447\u0435\u0440\u0435\u0434\u044C \u043D\u0435 \u0432\u044B\u043F\u043E\u043B\u043D\u044F\u0435\u0442\u0441\u044F."}`;
      for (const item of queue.items) {
        ...
        if (item.status === "uncertain")
          row.append(
            el(
              "p",
              "\u041F\u0440\u043E\u0432\u0435\u0440\u044C\u0442\u0435 \xAB\u041E\u0442\u043F\u0440\u0430\u0432\u043B\u0435\u043D\u043D\u044B\u0435\xBB \u0438 \u043E\u0442\u043A\u0440\u044B\u0442\u0443\u044E \u0432\u043A\u043B\u0430\u0434\u043A\u0443 \u043F\u0438\u0441\u044C\u043C\u0430. \u0421\u043A\u0440\u0438\u043F\u0442 \u043D\u0435 \u043F\u043E\u0432\u0442\u043E\u0440\u044F\u0435\u0442 \u044D\u0442\u0443 \u043E\u0442\u043F\u0440\u0430\u0432\u043A\u0443: \u043F\u0438\u0441\u044C\u043C\u043E \u0443\u0436\u0435 \u043C\u043E\u0433\u043B\u043E \u0443\u0439\u0442\u0438.",
              "note"
            )
          );
    ...
    async function start() {
      safe(() => {
        ...
        transport = new BrowserTransport(account, queueDelivery);
